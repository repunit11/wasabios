...
struct EfiGraphicsOutputProtocolPixelInfo {
    ...
    _padding0: [u32; 5],
    pub pixels_per_scan_line: u32, // 水平方向のデータに含まれる画素数 画面とフレームバッファの違い
}
...
struct EfiGraphicsOutputProtocolMode<'a> {
    ...
    pub frame_buffer_base: usize, // フレームバッファのメモリ領域の開始アドレス
    pub frame_buffer_size: usize, // フレームバッファの大きさ
}
...
fn efi_main(_image_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    fill_rect(&mut vram, 0x0000ff, 128, 128, 128, 128).expect("fill_rect feiled");
...
fn draw_line<T: Bitmap>(
    ...
    let dx = (x1 - x0).abs(); // 絶対値
    let sx = (x1 - x0).signum(); // 向き
    let dy = (y1 - y0).abs();
    let sy = (y1 - y0).signum();
    if dx >= dy {
